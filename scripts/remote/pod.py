    ...
    for name in ("id_ed25519.pub", "id_rsa.pub"):
        ...
        if os.path.exists(path):
            with open(path, "r") as f:
                return f.read().strip()
    raise PodError(
        f"no SSH public key found in {ssh_dir} (looked for id_ed25519.pub, "
...
if [ -z "${RUNPOD_POD_ID:-}" ] || [ -z "${RUNPOD_STOP_KEY:-}" ]; then
    echo "self-stop: RUNPOD_POD_ID or RUNPOD_STOP_KEY missing; cannot stop pod" >&2
    exit 1
fi
...
def wait_for_ready(pod_id, *, timeout_s=900, poll_s=10, probe=None, sdk=None,
