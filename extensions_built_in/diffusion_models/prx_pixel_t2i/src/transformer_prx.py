...
# small edits so it runs against a stock installed diffusers:
#   - the relative ``from ...`` / ``from ..`` imports are made absolute
#     (``from diffusers...``) -- every building block used here ships in stock
#     diffusers already, only the assembled PRX model is missing.
#   - ``maybe_adjust_dtype_for_device`` (added in a newer diffusers) is inlined
#     below so we don't depend on its presence.
#
# The pixel-space PRX-7B variant (Photoroom/prxpixel-t2i) sets
# ``in_channels=3`` (raw RGB, no VAE), ``bottleneck_size`` (two-layer patch
# projection) and ``resolution_embeds=True`` (resolution-aware modulation).
#
...
def maybe_adjust_dtype_for_device(
...
def get_image_ids(
    batch_size: int, height: int, width: int, patch_size: int, device: torch.device
) -> torch.Tensor:
    r"""
    ...
    return (
        img_ids.reshape((height // patch_size) * (width // patch_size), 2)
        .unsqueeze(0)
...
class PRXAttnProcessor2_0:
    ...
    def __init__(self):
        if not hasattr(torch.nn.functional, "scaled_dot_product_attention"):
            raise ImportError(
                "PRXAttnProcessor2_0 requires PyTorch 2.0, please upgrade PyTorch to 2.0."
            )
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if encoder_hidden_states is None:
            raise ValueError(
                "PRXAttnProcessor2_0 requires 'encoder_hidden_states' containing text tokens."
            )
        ...
        if attention_mask is not None:
            ...
            attn_mask_tensor = joint_mask[:, None, None, :].expand(
                -1, attn.heads, l_img, -1
            )
...
class PRXResolutionEmbedder(nn.Module):
    ...
    def forward(
        self, height: torch.Tensor, width: torch.Tensor, dtype: torch.dtype
    ) -> torch.Tensor:
...
class PRXBlock(nn.Module):
    ...
    def __init__(
        ...
        self.img_pre_norm = nn.LayerNorm(
    ...
    def forward(
        ...
        **kwargs: dict[str, Any],
        ...
        hidden_states_mod = (1 + attn_scale) * self.img_pre_norm(
            hidden_states
        ) + attn_shift
        ...
        hidden_states = hidden_states + mlp_gate * (
            self.down_proj(self.mlp_act(self.gate_proj(x)) * self.up_proj(x))
        )
...
class PRXTransformer2DModel(ModelMixin, ConfigMixin, AttentionMixin):
    ...
    Args:
        in_channels (`int`, *optional*, defaults to 16):
            Number of input channels in the latent image. The pixel-space PRX uses `3` (raw RGB, no VAE).
    ...
    def __init__(
        ...
        self.resolution_embedder = (
            PRXResolutionEmbedder(self.hidden_size, max_period=time_max_period)
            if resolution_embeds
            else None
        )
    ...
    def forward(
        ...
        Args:
            ...
            timestep (`torch.Tensor`):
                Timestep tensor of shape `(B,)` or `(1,)` on a `[0, 1]` scale, used for temporal conditioning.
            encoder_hidden_states (`torch.Tensor`):
        ...
        Returns:
            `Transformer2DModelOutput` if `return_dict=True`, otherwise a tuple `(sample,)` with the predicted clean
        ...
        img_ids = get_image_ids(
            bs, h, w, patch_size=self.patch_size, device=hidden_states.device
        )
        ...
        if self.resolution_embedder is not None:
            height = torch.full(
                (bs,), h, device=hidden_states.device, dtype=torch.float32
            )
            width = torch.full(
                (bs,), w, device=hidden_states.device, dtype=torch.float32
            )
