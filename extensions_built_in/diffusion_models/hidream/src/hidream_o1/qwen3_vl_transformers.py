...
class Qwen3VLTextRotaryEmbedding(nn.Module):
    ...
    def __init__(self, config: Qwen3VLTextConfig, device=None):
        ...
        self.config = config
        # Newer transformers removed "default" from ROPE_INIT_FUNCTIONS in
        # favor of a static method on the rotary module. Match that.
    ...
    def compute_default_rope_parameters(
        ...
        """Standard RoPE inv_freq. Mirrors upstream Qwen3VL since newer
        transformers removed "default" from ROPE_INIT_FUNCTIONS."""
    ...
    def forward(self, x, position_ids):
        ...
        if USE_BF16_ROPE == "1":
            ...
            inv_freq_expanded = (
                self.original_inv_freq[None, None, :, None]
                .float()
                .to(device=x.device)
                .expand(3, position_ids.shape[1], -1, 1)
            )
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if _mem_debug and (layer_idx % 4 == 0 or layer_idx == len(self.layers) - 1):
                ...
                print(
                    f"[MEM][rank{_rank}][decoder] layer {layer_idx:2d}/{len(self.layers)}: alloc={_a:.2f}GB",
                    flush=True,
                )
            ...
            if (
                return_mid_results_layers is not None
                and layer_idx in return_mid_results_layers
            ):
        ...
        if _mem_debug:
            _rank = int(_os.environ.get("RANK", 0))
            print(
                f"[MEM][rank{_rank}][decoder] LOOP END: alloc={_a:.2f}GB, GC_used={_gc_count}/{len(self.layers)} layers",
                flush=True,
            )
...
class BottleneckPatchEmbed(nn.Module):
    def __init__(
        self, config, patch_size=16, in_chans=3, pca_dim=768, embed_dim=768, bias=True
    ):
        super().__init__()
    ...
    def _forward_generation(
        ...
        **kwargs,
        ...
        vinputs_embedded = self.x_embedder(vinputs).to(
            inputs_embeds.dtype
        )  # [batch, img_tokens, hidden]
        inputs_embeds = torch.cat(
            [inputs_embeds, vinputs_embedded], dim=1
        )  # [batch, total_seq_len, hidden]
        ...
        if token_types.dim() == 1:
            ...
            token_types.dim() == 2
            and token_types.shape[-1] == 1
            and token_types.shape[0] == total_seq_len
        ):
        ...
        if use_flash_attn:
            ...
            attention_mask_4d = torch.stack(attn_masks, dim=0).unsqueeze(
                1
            )  # [batch, 1, seq, seq]
            ...
            if _mem_debug2:
                _rank2 = int(_os2.environ.get("RANK", 0))
                _mask_gb = (
                    attention_mask_4d.element_size() * attention_mask_4d.numel() / 1e9
                )
                _a = torch.cuda.memory_allocated() / 1e9
                print(
