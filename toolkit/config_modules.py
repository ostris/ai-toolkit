...
class SampleItem:
    def __init__(self, sample_config: "SampleConfig", **kwargs):
        ...
        self.seed: Optional[int] = kwargs.get(
            "seed", None
        )  # if none, default to autogen seed
        self.guidance_scale: float = kwargs.get(
        ...
        self.network_multiplier: float = kwargs.get(
        ...
        if isinstance(self.network_multiplier, str):
            try:
                ...
                print(
                    f"Invalid network_multiplier {self.network_multiplier}, defaulting to 1.0"
                )
...
class SampleConfig:
    def __init__(self, **kwargs):
        ...
        default_samples_kwargs = [{"prompt": x} for x in prompts]
...
class TrainConfig:
    def __init__(self, **kwargs):
        ...
        self.loss_type = kwargs.get(
            "loss_type", "mse"
        )  # mse, mae, wavelet, pixelspace, mean_flow
        ...
        self.latent_feature_extractor_path = kwargs.get(
            "latent_feature_extractor_path", None
        )
        ...
        self.diffusion_feature_extractor_path = kwargs.get(
            "diffusion_feature_extractor_path", self.latent_feature_extractor_path
        )
        self.diffusion_feature_extractor_weight = kwargs.get(
            "diffusion_feature_extractor_weight", self.latent_feature_loss_weight
        )
        ...
        self.guidance_loss_target: Union[int, List[int, int]] = kwargs.get(
...
class DatasetConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.control_path2: Union[str, List[str]] = kwargs.get(
            "control_path2", None
        )  # second control path
        if self.control_path2 == "":
            self.control_path2 = None
        self.control_path3: Union[str, List[str]] = kwargs.get(
            "control_path3", None
        )  # third control path
        ...
        self.control_transparent_color: List[int] = kwargs.get(
            "control_transparent_color", [0, 0, 0]
        )
        ...
        if (len(self.augments) > 0 or has_augmentations) and (
            ...
            print(
                "WARNING: Augments are not supported with caching latents. Setting cache_latents to False"
            )
        ...
        self.do_i2v: bool = kwargs.get(
            "do_i2v", True
        )  # do image to video on models that are both t2i and i2v capable
...
class GenerateImageConfig:
    def __init__(
        ...
        ctrl_img2: Optional[str] = None,  # second control image for multi-image models
        ctrl_img3: Optional[str] = None,  # third control image for multi-image models
        num_frames: int = 1,
        fps: int = 15,
        ctrl_idx: int = 0,
    ):
...
def validate_configs(
    ...
    if train_config.bypass_guidance_embedding and train_config.do_guidance_loss:
        raise ValueError(
            "Cannot bypass guidance embedding and do guidance loss at the same time. "
    ...
    is_caching_text_embeddings = any(
        dataset.cache_text_embeddings for dataset in dataset_configs
    )
    if is_caching_text_embeddings:
        ...
        if train_config.diff_output_preservation:
            raise ValueError(
                "Cannot use differential output preservation with caching text embeddings. Please set diff_output_preservation to False."
            )
        ...
        for dataset in dataset_configs:
            if not dataset.cache_text_embeddings:
                raise ValueError(
                    "All datasets must have cache_text_embeddings set to True when caching text embeddings is enabled."
                )

    # qwen image edit models cannot unload text encoder
    if model_config.arch in ["qwen_image_edit", "qwen_image_edit_plus2509"]:
        if train_config.unload_text_encoder:
            raise ValueError(
                "Cannot unload text encoder with qwen_image_edit models. Control images are encoded with text embeddings. You can cache the text embeddings though"
            )
