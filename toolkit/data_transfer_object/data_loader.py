        ...
        if use_db_entry:
            ...
            if self.dataset_config.fast_image_size:
                ...
                try:
                    ...
                    print_once(
                        "Warning: Some images in the dataset cannot be fast read. "
                        + "This process is faster for png, jpeg"
                    )
...
class DataLoaderBatchDTO:
    def __init__(self, **kwargs):
        try:
            ...
            if any(
                [
                    hasattr(x, "control_tensor2") and x.control_tensor2 is not None
                    for x in self.file_items
                ]
            ):
                # find one to use as a base
                ...
                for x in self.file_items:
                    if hasattr(x, "control_tensor2") and x.control_tensor2 is not None:
                        base_control_tensor2 = x.control_tensor2
                        break
                control_tensors2 = []
                for x in self.file_items:
                    if not hasattr(x, "control_tensor2") or x.control_tensor2 is None:
            ...
            if any(
                [
                    hasattr(x, "control_tensor3") and x.control_tensor3 is not None
                    for x in self.file_items
                ]
            ):
                # find one to use as a base
                ...
                for x in self.file_items:
                    if hasattr(x, "control_tensor3") and x.control_tensor3 is not None:
                        base_control_tensor3 = x.control_tensor3
                        break
                control_tensors3 = []
                for x in self.file_items:
                    if not hasattr(x, "control_tensor3") or x.control_tensor3 is None:
