...
class CaptionMixin:
    def get_caption_item(self: "AiToolkitDataset", index):
        ...
        if isinstance(img_path_or_tuple, tuple):
            img_path = (
                img_path_or_tuple[0]
                if isinstance(img_path_or_tuple[0], str)
                else img_path_or_tuple[0].path
            )
            ...
            img_path = (
                img_path_or_tuple
                if isinstance(img_path_or_tuple, str)
                else img_path_or_tuple.path
            )
        ...
        default_prompt_path_with_ext = os.path.join(
            os.path.dirname(img_path), "default" + ext
        )
        ...
        # handle replacements
        replacement_list = (
            self.dataset_config.replacements
            if isinstance(self.dataset_config.replacements, list)
            else []
        )
...
class BucketsMixin:
    ...
    def setup_buckets(self: "AiToolkitDataset", quiet=False):
        if not hasattr(self, "file_list"):
            raise Exception(
                f"file_list not found on class instance {self.__class__.__name__}"
            )
        if not hasattr(self, "dataset_config"):
            raise Exception(
                f"dataset_config not found on class instance {self.__class__.__name__}"
            )
        ...
        for idx, file_item in enumerate(file_list):
            ...
            if self.dataset_config.square_crop:
                ...
                if width > height:
                    file_item.crop_x = int(
                        file_item.scale_to_width / 2 - resolution / 2
                    )
                    file_item.crop_y = 0
                else:
                    file_item.crop_x = 0
                    file_item.crop_y = int(
                        file_item.scale_to_height / 2 - resolution / 2
                    )
            elif not did_process_poi:
                bucket_resolution = get_bucket_for_image_size(
                    width, height, resolution=resolution, divisibility=bucket_tolerance
                )
...
class CaptionProcessingDTOMixin:
    ...
    def get_caption(
        ...
        caption = inject_trigger_into_prompt(
            caption, trigger, to_replace_list, add_if_not_present
        )
        ...
        if self.dataset_config.random_triggers:
            ...
            if num_triggers > 0:
                triggers = random.sample(
                    self.dataset_config.random_triggers, num_triggers
                )
...
class ImageProcessingDTOMixin:
    def load_and_process_video(
        ...
        try:
            ...
            if (
                ...
                or total_frames < self.dataset_config.num_frames
            ):
                # Distribute frames evenly across the entire video
                interval = (
                    max_frame_index / (self.dataset_config.num_frames - 1)
                    if self.dataset_config.num_frames > 1
                    else 0
                )
                frames_to_extract = [
                    min(int(round(i * interval)), max_frame_index)
                    for i in range(self.dataset_config.num_frames)
                ]
            else:
                ...
                if max_consecutive_frames < self.dataset_config.num_frames:
                    ...
                    max_start_frame = max_frame_index - (
                        (self.dataset_config.num_frames - 1) * frame_interval
                    )
            ...
            frames_to_extract = [
                min(frame_idx, max_frame_index) for frame_idx in frames_to_extract
            ]
            ...
            for frame_idx in frames_to_extract:
                ...
                if hasattr(self.dataset_config, "debug") and self.dataset_config.debug:
                    ...
                    if actual_pos != frame_idx:
                        print_acc(
                            f"Warning: Failed to set exact frame position. Requested: {frame_idx}, Actual: {actual_pos}"
                        )
                ...
                if not ret:
                    ...
                    for fallback_offset in [1, -1, 5, -5, 10, -10]:
                        fallback_pos = max(
                            0, min(frame_idx + fallback_offset, max_frame_index)
                        )
                        ...
                        if fallback_ret:
                            ...
                            if (
                                hasattr(self.dataset_config, "debug")
                                and self.dataset_config.debug
                            ):
                                print_acc(
                                    f"Falling back to nearby frame {fallback_pos} instead of {frame_idx}"
                                )
                        ...
                        raise Exception(
                            f"Failed to read frame {frame_idx} from video. {frame_pos_info}. {video_info}"
                        )
            ...
            if hasattr(self.dataset_config, "debug") and self.dataset_config.debug:
                print_acc(
                    f"Successfully loaded video with {len(frames)} frames: {self.path}"
                )
            ...
            try:
                if "Failed to read frame" in error_msg and cap is not None:
                    ...
                    print_acc("Video details when error occurred:")
                    ...
                    if cap.isOpened():
                        ...
                        if reported_total > 0:
                            ...
                            end_ret, _ = cap.read()
                            print_acc(
                                f"  Can read first frame: {start_ret}, Can read last frame: {end_ret}"
                            )
    ...
    def load_and_process_image(
        ...
        if self.is_latent_cached:
            self.get_latent()
            if (
                self.has_control_image
                or self.has_control_image2
                or self.has_control_image3
            ):
        ...
        if self.dataset_config.buckets:
            ...
            if (
                img.width < self.crop_x + self.crop_width
                or img.height < self.crop_y + self.crop_height
            ):
            ...
            img = img.resize(
                (
                    int(img.size[0] * self.dataset_config.scale),
                    int(img.size[1] * self.dataset_config.scale),
                ),
                Image.BICUBIC,
            )
            ...
            if self.dataset_config.random_crop:
                if (
                    self.dataset_config.random_scale
                    and min_img_size > self.dataset_config.resolution
                ):
                    if min_img_size < self.dataset_config.resolution:
                        ...
                        scale_size = random.randint(
                            self.dataset_config.resolution, int(min_img_size)
                        )
                ...
                img = img.resize(
                    (self.dataset_config.resolution, self.dataset_config.resolution),
                    Image.BICUBIC,
                )
...
class ControlFileItemDTOMixin:
    def __init__(self: "FileItemDTO", *args, **kwargs):
        ...
        # Handle control_path2
        if dataset_config.control_path2 is not None:
            control_path2_list = dataset_config.control_path2
            if not isinstance(control_path2_list, list):
                control_path2_list = [control_path2_list]

            img_path = kwargs.get("path", None)
            file_name_no_ext = os.path.splitext(os.path.basename(img_path))[0]

            found_control_images2 = []
            for control_path in control_path2_list:
                for ext in img_ext_list:
                    if os.path.exists(
                        os.path.join(control_path, file_name_no_ext + ext)
                    ):
                        found_control_images2.append(
                            os.path.join(control_path, file_name_no_ext + ext)
                        )
                        self.has_control_image2 = True
                        break
            self.control_path2 = found_control_images2
            if len(self.control_path2) == 0:
                self.control_path2 = None
            elif len(self.control_path2) == 1:
                self.control_path2 = self.control_path2[0]

        # Handle control_path3
        if dataset_config.control_path3 is not None:
            control_path3_list = dataset_config.control_path3
            if not isinstance(control_path3_list, list):
                control_path3_list = [control_path3_list]

            img_path = kwargs.get("path", None)
            file_name_no_ext = os.path.splitext(os.path.basename(img_path))[0]

            found_control_images3 = []
            for control_path in control_path3_list:
                for ext in img_ext_list:
                    if os.path.exists(
                        os.path.join(control_path, file_name_no_ext + ext)
                    ):
                        found_control_images3.append(
                            os.path.join(control_path, file_name_no_ext + ext)
                        )
                        self.has_control_image3 = True
                        break
            self.control_path3 = found_control_images3
            if len(self.control_path3) == 0:
                self.control_path3 = None
            elif len(self.control_path3) == 1:
                self.control_path3 = self.control_path3[0]
...
class ClipImageFileItemDTOMixin:
    def __init__(self: "FileItemDTO", *args, **kwargs):
        ...
        dataset_config: "DatasetConfig" = kwargs.get("dataset_config", None)
        if (
            dataset_config.clip_image_path is not None
            or dataset_config.clip_image_from_same_folder
        ):
        ...
        if dataset_config.clip_image_path is not None:
            ...
            for ext in img_ext_list:
                if os.path.exists(
                    os.path.join(clip_image_path, file_name_no_ext + ext)
                ):
                    self.clip_image_path = os.path.join(
                        clip_image_path, file_name_no_ext + ext
                    )
    ...
    def build_clip_imag_augmentation_transform(self: "FileItemDTO"):
        if (
            self.dataset_config.clip_image_augmentations is not None
            and len(self.dataset_config.clip_image_augmentations) > 0
        ):
            self.has_clip_augmentations = True
            augmentations = [
                Augments(**aug) for aug in self.dataset_config.clip_image_augmentations
            ]
            ...
            for aug in augmentations:
                ...
                assert hasattr(A, aug.method_name), (
    ...
    def augment_clip_image(
        ...
        if self.clip_vision_is_quad:
            ...
            # recombine
            augmented = np.vstack(
                (np.hstack((img1_1, img1_2)), np.hstack((img2_1, img2_2)))
            )
        ...
        augmented_tensor = (
            transforms.ToTensor()(augmented)
            if transform is None
            else transform(augmented)
        )
    ...
    def get_clip_vision_embeddings_path(self: "FileItemDTO", recalculate=False):
        if self._clip_vision_embeddings_path is not None and not recalculate:
            ...
            filename_no_ext = os.path.splitext(os.path.basename(self.clip_image_path))[
                0
            ]
            ...
            hash_str = base64.urlsafe_b64encode(
                hashlib.md5(hash_input).digest()
            ).decode("ascii")
            hash_str = hash_str.replace("=", "")
            self._clip_vision_embeddings_path = os.path.join(
                latent_dir, f"{filename_no_ext}_{hash_str}.safetensors"
            )
    ...
    def load_clip_image(self: "FileItemDTO"):
        is_dynamic_size_and_aspect = isinstance(
            self.clip_image_processor, PixtralVisionImagePreprocessorCompatible
        ) or isinstance(self.clip_image_processor, SiglipImageProcessor)
        ...
        try:
            ...
            img = Image.new(
                "RGB", (self.dataset_config.resolution, self.dataset_config.resolution)
            )
            print_acc(f"Error: {e}")
...
class AugmentationFileItemDTOMixin:
    ...
    def build_augmentation_transform(self: "FileItemDTO"):
        if (
            self.dataset_config.augmentations is not None
            and len(self.dataset_config.augmentations) > 0
        ):
            self.has_augmentations = True
            augmentations = [
                Augments(**aug) for aug in self.dataset_config.augmentations
            ]
            ...
            self.aug_transform = A.ReplayCompose(
                augmentation_list, additional_targets={"image2": "image"}
            )
    ...
    def augment_image(
        ...
        self.unaugmented_tensor = (
            transforms.ToTensor()(img) if transform is None else transform(img)
        )
        ...
        augmented_params["transforms"] = [
            t
            for t in augmented_params["transforms"]
            if t["__class_fullname__"].split(".")[-1] in spatial_transforms
        ]
    ...
    def augment_spatial_control(
        self: "FileItemDTO", img: Image, transform: Union[None, transforms.Compose]
    ):
        ...
        transformed = A.ReplayCompose.replay(
            self.aug_replay_spatial_transforms, image=open_cv_image
        )
        augmented = transformed["image"]
...
class MaskFileItemDTOMixin:
    def __init__(self: "FileItemDTO", *args, **kwargs):
        ...
        if dataset_config.alpha_mask:
            ...
            # find the control image path
            mask_path = (
                dataset_config.mask_path
                if dataset_config.mask_path is not None
                else dataset_config.alpha_mask
            )
            # we are using control images
            img_path = kwargs.get("path", None)
            file_name_no_ext = os.path.splitext(os.path.basename(img_path))[0]
    ...
    def load_mask_image(self: "FileItemDTO"):
        ...
        if fix_size:
            # swap all the sizes
            self.scale_to_width, self.scale_to_height = (
...
class UnconditionalFileItemDTOMixin:
    def __init__(self: "FileItemDTO", *args, **kwargs):
        ...
        if dataset_config.unconditional_path is not None:
            ...
            for ext in img_ext_list:
                if os.path.exists(
                    os.path.join(
                        dataset_config.unconditional_path, file_name_no_ext + ext
                    )
                ):
                    self.unconditional_path = os.path.join(
                        dataset_config.unconditional_path, file_name_no_ext + ext
                    )
    ...
    def load_unconditional_image(self: "FileItemDTO"):
        ...
        if self.dataset_config.buckets:
            ...
            raise Exception(
                "Unconditional images are not supported for non-bucket datasets"
            )

        if self.aug_replay_spatial_transforms:
            self.unconditional_tensor = self.augment_spatial_control(
                img, transform=self.unconditional_transforms
            )
        else:
    ...
    def __init__(self: "FileItemDTO", *args, **kwargs):
        ...
        if self.poi is not None:
            ...
            if dataset_config.cache_latents or dataset_config.cache_latents_to_disk:
                raise Exception(
                    "Error: poi is not supported when caching latents. Please set cache_latents and cache_latents_to_disk to False in the dataset config"
                )
                # make sure we are loading through json
            if dataset_config.caption_ext != "json":
                raise Exception(
                    "Error: poi is only supported when using json captions. Please set caption_ext to json in the dataset config"
                )
            ...
            if not os.path.exists(caption_path):
                raise Exception(
                    f"Error: caption file not found for poi: {caption_path}"
                )
    ...
    def setup_poi_bucket(self: "FileItemDTO"):
        ...
        while True:
            ...
            if current_resolution >= self.dataset_config.resolution:
                ...
                if num_loops > 100:
                    print_acc(
                        "Warning: poi bucketing looped too many times. This should not happen. Please report this issue."
                    )
                    return False
        ...
        if (
            self.scale_to_width < self.crop_x + self.crop_width
            or self.scale_to_height < self.crop_y + self.crop_height
        ):
            # todo look into this. This still happens sometimes
            print_acc("size mismatch")
...
class LatentCachingFileItemDTOMixin:
    ...
    def get_latent_path(self: "FileItemDTO", recalculate=False):
        if self._latent_path is not None and not recalculate:
            ...
            hash_str = hash_str.replace("=", "")
            self._latent_path = os.path.join(
                latent_dir, f"{filename_no_ext}_{hash_str}.safetensors"
            )
            ...
            for file_item in tqdm(
                ...
                if os.path.exists(latent_path):
                    if to_memory:
                        ...
                        file_item._encoded_latent = state_dict["latent"].to(
                            "cpu", dtype=self.sd.torch_dtype
                        )
                else:
                    # not saved to disk, calculate
                    # load the image first
                    file_item.load_and_process_image(
                        self.transform, only_load_latents=True
                    )
                    ...
                    if to_memory:
                        # keep it in memory
                        file_item._encoded_latent = latent.to(
                            "cpu", dtype=self.sd.torch_dtype
                        )
...
class TextEmbeddingFileItemDTOMixin:
    ...
    def get_text_embedding_path(self: "FileItemDTO", recalculate=False):
        if self._text_embedding_path is not None and not recalculate:
            ...
            hash_str = hash_str.replace("=", "")
            self._text_embedding_path = os.path.join(
                te_dir, f"{filename_no_ext}_{hash_str}.safetensors"
            )
...
class TextEmbeddingCachingMixin:
    ...
    def cache_text_embeddings(self: "AiToolkitDataset"):
        with accelerator.main_process_first():
            ...
            i = 0
            for file_item in tqdm(
                self.file_list, desc="Caching text embeddings to disk"
            ):
    ...
    def cache_clip_vision_to_disk(self: "AiToolkitDataset"):
        ...
        with torch.no_grad():
            ...
            if self.sd.adapter is None:
                raise Exception(
                    "Error: must have an adapter to cache clip vision to disk"
                )
            ...
            if clip_image_processor is None:
                raise Exception(
                    "Error: must have a clip image processor to cache clip vision to disk"
                )
            ...
            if vision_encoder is None:
                raise Exception(
                    "Error: must have a vision encoder to cache clip vision to disk"
                )
            ...
            device = self.sd.device_torch
            if (
                hasattr(self.sd.adapter, "clip_noise_zero")
                and self.sd.adapter.clip_noise_zero
            ):
                # just to do this, we did :)
                # need more samples as it is random noise
                self.clip_vision_num_unconditional_cache = (
                    self.clip_vision_num_unconditional_cache
                )
            else:
            ...
            print_acc(
                f" - Caching {self.clip_vision_num_unconditional_cache} unconditional clip vision to disk"
            )
            clip_vision_cache_path = os.path.join(
                self.dataset_config.clip_image_path, "_clip_vision_cache"
            )
            ...
            for i in range(self.clip_vision_num_unconditional_cache):
                ...
                uncond_path = os.path.join(
                    clip_vision_cache_path, f"uncond_{hash_str}_{i}.safetensors"
                )
                ...
                clip_output = vision_encoder(
                    clip_image.to(device, dtype=dtype), output_hidden_states=True
                )
                ...
                state_dict = OrderedDict(
                    [
                        (
                            "image_embeds",
                            clip_output.image_embeds.clone().detach().cpu(),
                        ),
                        (
                            "last_hidden_state",
                            clip_output.hidden_states[-1].clone().detach().cpu(),
                        ),
                        (
                            "penultimate_hidden_states",
                            clip_output.hidden_states[-2].clone().detach().cpu(),
                        ),
                    ]
                )
            ...
            # use tqdm to show progress
            i = 0
            for file_item in tqdm(self.file_list, desc="Caching clip vision to disk"):
                ...
                if file_item.has_clip_augmentations:
                    raise Exception(
                        "Error: clip vision caching is not supported with clip augmentations"
                    )

                embedding_path = file_item.get_clip_vision_embeddings_path(
                    recalculate=True
                )
                # check if it is saved to disk already
                if not os.path.exists(embedding_path):
                    ...
                    # add batch dimension
                    clip_image = file_item.clip_image_tensor.unsqueeze(0).to(
                        device, dtype=dtype
                    )
                    ...
                    # metadata
                    meta = get_meta_for_safetensors(
                        file_item.get_clip_vision_info_dict()
                    )
...
class ControlCachingMixin:
    ...
    def add_control_path_to_file_item(
        ...
        if control_type == "inpaint":
            ...
            if file_item.control_path is None:
                ...
                raise Exception(
                    f"Error: control_path is not a string or list: {file_item.control_path}"
                )
    ...
    def setup_controls(self: "AiToolkitDataset"):
        ...
        with torch.no_grad():
            ...
            # use tqdm to show progress
            for file_item in tqdm(self.file_list, desc="Generating Controls"):
                for control_type in self.dataset_config.controls:
                    # generates the control if it is not already there
                    control_path = self.control_generator.get_control_path(
                        file_item.path, control_type
                    )
                    if control_path is not None:
                        self.add_control_path_to_file_item(
                            file_item, control_path, control_type
                        )
