...
const runDownload = (downloadId: string, url: string, outputDir: string): void => {
  const ytDlpPath = getYtDlpPath();

  const proc = spawn(ytDlpPath, ['--newline', '-o', path.join(outputDir, '%(title)s.%(ext)s'), url]);
  ...
  proc.on('error', (err: Error) => {
    prisma.videoDownload
      .update({ where: { id: downloadId }, data: { status: 'failed', error: err.message } })
...
export default async function processDownloads() {
  ...
  for (const download of pending) {
    ...
    runDownload(download.id, download.url, outputDir);
  }
}
