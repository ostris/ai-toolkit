...
async function getAMDGpuStats(isWindows: boolean) {
  ...
  const sdata = JSON.parse(data[0]);
  const mdata = JSON.parse(data[1]);

  var gpus = sdata["gpu_data"].map(d => {
    const i = parseInt(d["gpu"]);
    const gpu_data = mdata["gpu_data"][i];
    const mem_total = parseFloat(gpu_data["mem_usage"]["total_vram"]["value"]);
    const mem_used =  parseFloat(gpu_data["mem_usage"]["used_vram"]["value"]);
    const mem_free =  parseFloat(gpu_data["mem_usage"]["free_visible_vram"]["value"]);
    ...
    return {
      ...
      temperature: parseInt(gpu_data["temperature"]["hotspot"]["value"]),
      utilization: {
        gpu: parseInt(gpu_data["usage"]["gfx_activity"]["value"]),
      ...
      power: {
        draw: parseFloat(gpu_data["power"]["socket_power"]["value"]),
        limit: parseFloat(d["limit"]["max_power"]["value"]),
      },
      clocks: {
        graphics: parseInt(gpu_data["clock"]["gfx_0"]["clk"]["value"]),
        memory: parseInt(gpu_data["clock"]["mem_0"]["clk"]["value"]),
      },
      fan: {
        speed: parseFloat(gpu_data["fan"]["usage"]["value"]), // Some GPUs might not report fan speed, default to 0
      }
    };
  });
