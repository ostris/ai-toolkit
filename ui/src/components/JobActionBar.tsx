...
export default function JobActionBar({
  ...
  return (
    <div className={`flex items-center flex-shrink-0 ${className ?? ''}`}>
      ...
      <Button
        onClick={() => {
          let message = `确定要删除任务“${job.name}”吗？这也将永久从磁盘中移除。`;
          if (job.status === 'running') {
            message += ' 警告：任务当前正在运行，如果可能您应先停止它。';
          }
          openConfirm({
            title: '删除任务',
            message: message,
        ...
        >
          ...
          <MenuItem>
            <div
              className="cursor-pointer px-4 py-1 hover:bg-gray-800 rounded flex items-center gap-2"
              onClick={() => {
                let message = `确定要将此任务标记为已停止吗？如果状态已卡住，这将把任务状态设置为“已停止”。仅当您 100% 确定任务已停止时才执行此操作。这不会停止任务。`;
                openConfirm({
                  title: '标记任务为已停止',
                  message: message,
                  type: 'warning',
